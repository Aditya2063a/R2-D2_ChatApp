...
const MessageBar = () => {
  ...
  const handleSendMessage = async () => {
    ...
    if (message.startsWith("@ai")) {
      ...
      const aiQuery = message.slice(3).trim();
      ...
      if (aiQuery) {
        ...
        try {
          ...
          // Call your AI API or service with the query
          const aiResponse = await apiClient.post("/ai-chat/ask", { question: aiQuery });

          let aiAnswer = aiResponse.data.answer;
  
          // Emit the AI response as a message to the chat
          if (selectedChatType === "contact") {
          socket.emit("sendMessage", {
            sender: userInfo.id,
            content: ` 	(｡◕‿‿◕｡)    : \n${aiAnswer}`,
